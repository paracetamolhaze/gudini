...
Вступительный ХУК может идти без картинки: выбери по смыслу, когда завершилась завязка.
После хука СВЕРХУ КАРТИНКА ДОЛЖНА БЫТЬ ВСЕГДА до последнего кадра. Дальше пустых участков нет.
...
Только во вступительном хуке можно оставлять автора без картинки; нельзя тянуть случайную фотографию через смену темы.
...
Первая quote начинается с первых слов транскрипции, если хук нуждается в иллюстрации,
или с первых слов ПОСЛЕ вступительного хука, если его лучше оставить без картинки.
Не откладывай первую картинку за пределы хука. Границу хука выбирай по смыслу речи.
...
Не делай новую смену ради другого ракурса той же машины. ПОВТОРЫ КАРТИНОК ЗАПРЕЩЕНЫ:
каждый assetId используй только один раз за весь ролик, даже при возврате сюжета к предмету.
Не используй копии одной фотографии под разными id. Выбирай новый объясняющий аспект.
...
export async function directMontage(
  ...
  const raw = await mediaComplete({
    ...
    user:
      ...
      `Хук может быть без картинки; после него верх заполнен до конца. Ни одна картинка не повторяется. Выбирай смысловые эпизоды и точки смены.\n\n` +
...
export function planSemanticSequence(
  ...
  for (const p of placements) {
    ...
    if (events.some(e => e.assetId === asset.id)) throw new Error(`Повтор картинки запрещён: ${asset.id}`);
    const start = wordIndex === 0 ? 0 : words[wordIndex].start;
