...
- **X** — API v2 на OAuth 1.0a, платный по факту: каждый вызов пишется в `platform_usage`, чтение чужих постов
  ограничено дневным бюджетом. С 23.02.2026 X принимает ответы через API только авторам, которые упомянули или
  процитировали аккаунт, поэтому: ответы под **своими** постами идут через API; для **чужих** постов текст
  готовится, а отправляет его владелец по ссылке «Открыть в X» (режим `manual`) либо он выходит цитатой (`quote`).
  Пост со ссылкой в X стоит на порядок дороже, поэтому ссылки по умолчанию вырезаются.
