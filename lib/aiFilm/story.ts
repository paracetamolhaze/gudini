...
Персонажи истории (supportingCharacters): те, о ком говорит автор, до 6 значимых на ролик; у каждого функция opponent, guide, witness, partner или background и узнаваемое описание внешности по iconic-признакам (без имени франшизы в appearance). Не создавать толпы похожих людей, не путать, кто главный в кадре.
...
Каждая AI-сцена: ONE SHOT = ONE CLEAR ACTION, понятная за 1–2 секунды. visualAction (английский) обязан содержать WHO (${character.name}), WHAT HE DOES, WHERE, WHAT CHANGES. Плохо: "${character.name} reflects on uncertainty while symbolic lights shift". Хорошо: "${character.name} enters an empty training ground. Every target post has fallen except one. He slowly picks up the single scroll left on it." Без десяти действий сразу, без сюрреалистического мусора, без текста/надписей/логотипов в кадре, без крови и графического насилия. Известные персонажи и реальные люди в кадре рисуются как персонажи аниме, узнаваемо по iconic-признакам. В visualAction, location, stateBefore/After и appearance НЕ пиши названия франшиз, студий, фильмов и имена персонажей (Marvel, Avengers, Iron Man, Thanos, Doom и т.п.) — генератор видео их фильтрует; пиши визуальное описание: «the red-and-gold armored hero with a glowing chest reactor», «a giant purple titan with a golden gauntlet», «the soldier with a silver metal arm», «a hero with a round star-spangled shield». В meaning, storyBeat и universeAdaptation имена писать можно — это пояснение для автора.
