...
- scene.storyObject: РОВНО ОДИН главный сюжетный объект, ≤5 слов (не список символов).
  Объект НЕ должен быть носителем текста: не вывеска, не табличка, не экран с надписью, не
  постер, не газета, не бегущая строка, не билет с печатью, не логотип. Генератор нарисует на
  них буквы, и проверка отклонит обложку («SOLD OUT» на табличке уже провалил обложку).
  Сюжет про билеты — пустые кресла, толпа у входа, рука с билетом без букв; про деньги —
  купюры размытым фоном, а не табло с цифрами.
