...
чтобы видимость выбрал сам автор, без формы Direct Post не проходит аудит. **«Отправить в черновики»**: YouTube приватным черновиком (виден только в Studio), TikTok в черновики
...
## 9. Обложка на YouTube Shorts и TikTok

Оба показывают у Shorts только кадр из видео: YouTube принимает картинку через API, но для Shorts
её не использует, TikTok свою картинку не принимает вовсе. Поэтому для них публикуется копия ролика
`out-lead.mp4`, где обложка стоит первым кадром `COVER_LEAD_SEC` секунд (0.2 по умолчанию, 0 —
выключить): превью становится своей обложкой, зритель видит короткую вспышку картинки в начале.
Копия делается один раз и переиспользуется, пока не изменились ролик или обложка. Instagram получает
оригинал и обложку по ссылке. Для TikTok кадр обложки в форме по умолчанию 0 — это и есть картинка.

## 10. Обложка YouTube Shorts: что делает сайт и где предел API

Через API нельзя выбрать кадр превью для Shorts, и свою картинку в ленте Shorts YouTube не
показывает — это документированное ограничение, оно же у всех сервисов отложенного постинга.
Сайт делает максимум, что даёт API: ставит обложку при загрузке, затем ждёт конца обработки
(`videos.list`, scope `youtube.readonly` — переподключить YouTube один раз) и ставит её ещё дважды;
результат дописывается в карточку публикации. Плюс обложка первым кадром (`COVER_LEAD_SEC`).
Если после этого в ленте Shorts всё равно автокадр — это выбор YouTube, поменять его можно только в
Studio, выбрав кадр вручную.
