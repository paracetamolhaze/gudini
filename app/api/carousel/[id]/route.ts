...
/** Ручные правки: тексты, порядок, подпись, хэштеги, место текста и версия иллюстрации, оформление. Генератор не вызывается. */
export async function PATCH(req: NextRequest, { params }: Ctx) {
  ...
  try {
    ...
    const design = body.applyDesign === true ? readDesign() : undefined;
    let render = false;
    const c = updateCarousel(id, (x) => {
      ...
      const { contentChanged, renderNeeded } = applyManualEdit(x, body, { design });
