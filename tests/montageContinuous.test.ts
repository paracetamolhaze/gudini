...
test('production requires a picture from the first frame through the last frame', () => {
...
test('semantic quote boundaries determine durations without a timer or automatic extra cuts', () => {
  ...
  assert.throws(() => planSemanticSequence(placements.slice(1), { assets } as any, words, 25), /начало/);
