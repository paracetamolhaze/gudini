...
/**
 * Сколько результат обязан держаться на экране. Появление результата, длительность его показа
 * и конец монтажного отрезка — три разных времени, и раньше в плане было только первое:
 * подтверждение на экране устройства успевало смениться обратно внутри того же клипа.
 */
export type HoldKind = "instant" | "settle" | "read";

/** Удержание в секундах: мгновенное, до состояния покоя, до прочтения. */
export const HOLD_SECONDS: Record<HoldKind, number> = { instant: 0, settle: 1, read: 2 };
...
export type StoryEvent = {
  ...
  objects: ObjectState[];
  /**
   * Статус факта относительно справки: confirmed — справка подтверждает (basisFact — её фраза);
   * told — есть только в речи автора; contradicted — справка говорит иначе (basisFact — её фраза).
   * Механизм с неподтверждённым статусом — кто распознал, кто решил, кто сказал — изображать
   * нельзя: показывается только внешне наблюдаемый исход, о механизме говорит голос.
   */
...
/**
 * Визуальная задача истории: что зритель должен увидеть за весь ролик, а не под одну реплику.
 * Составляется до раскладки по битам. event — сцена показывает само сказанное; illustration —
 * место, масштаб или обстановку, не доказывая утверждения; explanation — правовая, числовая
 * или причинная связь без честной картинки, её несёт автор.
 */
export type VisualTask = {
  id: string;
  /** что зритель узнаёт из этой картинки, русский */
  learns: string;
  role: "event" | "illustration" | "explanation";
  fromPhrase: number;
  toPhrase: number;
  /** одно выполнимое наблюдаемое действие, английский; у объяснения пусто */
  action: string;
  /** секунды речи, где звучит задача; проставляются по фразам */
  start?: number;
  end?: number;
};
...
export type StoryBible = {
  ...
  /** визуальные задачи всей истории; пусто — план построен без этой процедуры */
  visualTasks?: VisualTask[];
  /** факты справки по теме, по которым разбор проверяет статус событий; пусто — справки не было */
  researchFacts?: string[];
  /**
   * Обязательные события, которые несёт голос автора: планировщик сам отдал их реплики
   * объяснению. Обязательное для понимания не всегда обязательно к показу — правовой исход
   * или сумму честно не снять. Берётся из первого ответа, второй заход его не расширяет.
   */
  authorCarried?: string[];
};
...
export type StoryBeat = {
  ...
  keyMoment: string;
  /**
   * Сколько результат обязан оставаться на экране ПОСЛЕ появления. Три разных времени
   * не одно и то же: когда результат появился, сколько он держится и где кончается
   * монтажный отрезок. Удар и вспышка мгновенны, подтверждение и надпись нужно успеть
   * прочитать, упавший предмет должен прийти в покой.
   */
  hold: HoldKind;
  /**
  ...
  eventIds: string[];
  /** какую визуальную задачу истории решает сцена; у авторского бита пусто */
  visualTask?: string;
  ...
  /**
   * ЧТО занимает кадр и обязано читаться, короткой именной группой на английском:
   * лицо, расстояние между двумя людьми, место контакта пальца с кнопкой, пустой зал.
   * Выбирается планировщиком по смыслу сцены; крупность и ракурс относятся именно к нему.
   */
  frameSubject: string;
  /** где субъект кадра и для чего оставлено место */
...
export type EventDeadline = {
  ...
  eventIds: string[];
  keyMoment: string;
  /** не позже этой секунды клипа результат обязан появиться */
  bySec: number | null;
  /** сколько секунд он обязан держаться после появления: 0 для мгновенного */
  holdSec: number;
  /** до какой секунды клипа он обязан остаться на экране; null — держать нечего */
  untilSec: number | null;
  /** удержание не помещается в используемый отрезок: зритель не успеет прочитать результат */
  tight?: boolean;
  beyond?: boolean;
};
...
export type TimelineSegment = {
  ...
  beatIds: string[];
  /**
   * С какой секунды клипа группы берётся материал окна. Место вставки на таймлайне и начало
   * используемого материала — независимые величины: удачный отрезок единственного дубля можно
   * поставить под нужные слова, не двигая голос. Пусто — отсчёт от начала группы, как раньше.
   */
  clipIn?: number;
};
