...
ШАГ 1. Пойми историю и её причинные связи, потом выпиши СОБЫТИЯ (bible.events). Событие — то, без чего рассказ разваливается: то, что произошло, а не то, о чём рассуждают.
Перед списком ответь себе: что зритель обязан понять; кто участвует; какие предметы существенны; что чему предшествует. Предмет сначала получают, потом используют; следствие идёт после причины; согласие предшествует передаче. Порядок рассказа и порядок событий могут не совпадать: если речь возвращается назад, это ретроспектива, а не события в обратном порядке.
...
- легенда, слух, пересказ — показывай, как рассказывают или как выглядит источник рассказа, а не само событие как состоявшееся;
- предположение и расчёт — показывай основание расчёта, а не его результат как факт;
...
- совершение: ключ переходит из руки в руку; дверь закрывают; стакан опрокидывается от удара; человек встаёт и уступает место;
- уже готовый результат: ключ лежит у второго; дверь закрыта; осколки на полу; другой человек уже сидит;
- подготовка: рука тянется к ключу; человек стоит у двери; мяч лежит на столе.
Результат и подготовка — нормальные сцены, когда истории нужен именно результат или контекст. Но событием «совершил» они не считаются, и в eventIds такого события не ссылаются.
У каждой сцены своё назначение: purpose говорит, показывает она событие, его результат, реакцию или обстановку.
...
ПРОВЕРЬ КАЖДУЮ СЦЕНУ ПЕРЕД ОТВЕТОМ, четыре вопроса:
1. Видно ли обещанное — есть ли в кадре поверхность, деталь или контакт, по которым зритель это заключит?
2. Успеет ли оно произойти за показанное время, считая зависимые шаги?
3. Не спорят ли между собой камера, реквизит, состояние предметов и внешность участников?
4. Не утверждает ли кадр больше, чем сказано в речи: обещанное как сделанное, легенда как факт, догадка как документ?
Если на любой вопрос ответ плохой, перепиши сцену до ответа, а не добавляй оговорку.
...
═══ 6. НАБЛЮДАЕМОЕ ДОКАЗАТЕЛЬСТВО СЦЕНЫ ═══
Порядок работы над каждой сценой ровно такой: смысл и статус утверждения → роль кадра (событие, результат, реакция, обстановка) → видимое доказательство → исходные условия → короткое выполнимое действие → камера, которая это доказательство видит → монтажное окно.
На каждую сцену ответь: КАКОЙ конкретный момент даст зрителю понять, что произошло? Это может быть контакт, передача, изменение формы, завершение действия, реакция человека, контраст двух состояний или значимая деталь обстановки.
Этот момент и пиши в keyMoment. Если назвать его нечем — сцены нет, отдай бит автору.
Восемь секунд «стоит, держит, смотрит, ждёт, поправляет, готовится» главным содержанием сцены не годятся: это подводка. Осмысленная пауза или реакция допустима, когда она и есть доказательство (человек услышал отказ и опускает руку).
Не дроби одно событие на несколько платных сцен: «разбежался, прыгнул, перевернулся, приземлился» — одна сцена. Но и не превращай событие в неподвижную позу.
...
- scene.worn (список) — что надето или закреплено на человеке и обязано остаться между сценами: рюкзак, шлем, повязка, фартук, сумка через плечо;
- scene.props (список) — реквизит, который обязан быть в кадре, даже если сам не меняется: тот самый конверт, ключи на столе, инструмент в руке;
- scene.mechanics (английский) — что запускает действие, что с чем соприкасается, как меняется опора, нагрузка или препятствие.
Предмет, нужный действию, не исчезает только потому, что не меняет собственного состояния: назови его в scene.props или scene.worn, и он останется в кадре.
Непрерывность: повреждённое не появляется до повреждения и не становится целым само. Обратимое изменение разрешено, если в кадре есть причина: дверь можно закрыть, шар — сдуть, человек может вернуться в прежнее место. Без причины возврат читается как ошибка.
stateBefore / stateAfter (английский, одна строка) — короткая сводка ОСТАЛЬНОГО: свет, обстановка, расстановка участников. Не повторяй в ней состояния из objects.

═══ 8a. МЕХАНИКА ДЕЙСТВИЯ ═══
Для сцен с движением, контактом или изменением материала опиши в scene.mechanics и motion то, что видно:
- что запускает действие и что с чем соприкасается;
- как движутся тело и предметы относительно окружения;
- как материал меняет форму: рвётся по шву под нагрузкой, мнётся, разливается, осыпается, останавливается;
- как смена опоры, нагрузки или препятствия меняет движение: пока верёвка не натянулась — свободное падение, после — подвес и торможение.
Слова «real physics», «natural anatomy», «cinematic» ничего не описывают и кадр не спасают. Пиши положительные наблюдаемые указания; короткие запреты ставь только к тем ошибкам, которые в этой сцене вероятны.
...
Одно мотивированное движение камеры либо неподвижная камера. Требования про ветер, обломки, пыль и прочую физику ставь ТОЛЬКО той сцене, где это происходит.
Если камера сопровождает движущийся объект, назови видимый ориентир, по которому читается скорость: стена, деревья, край дороги. Без ориентира сопровождение выглядит как зависание.
Не направляй объект в объектив и одновременно не требуй постоянной крупности: либо камера отступает, либо объект идёт мимо неё.
Плохо: "${character.name} reflects on uncertainty while symbolic lights shift" — нет ни действия, ни точки съёмки.
Плохо: "Camera is above him looking straight down; the important thing is above his head" — важное окажется между ним и камерой и закроет кадр.
Хорошо: "Camera is beside the table at hand height; his hand pushes the glass across frame left to right and it tips over the edge" — виден контакт и результат.
Хорошо: "Camera stays level with the doorway three meters back; the courier holds out the sealed envelope and the other man takes it, both hands in frame" — видна передача.
