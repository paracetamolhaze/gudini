...
МЕХАНИЗМ — кто и как распознал, кто принял решение, кто позвонил, кто сказал пассажирам — В ДОКУМЕНТАЛЬНОЙ ИСТОРИИ (news, history) при basis told или contradicted изображать нельзя: у такого события показывается только внешне наблюдаемый исход (машина остановилась, дверь открыли, люди подошли), а о механизме говорит голос. Обязательность события для рассказа не делает его обязательным к показу. Отсутствие сведений — не опровержение: told означает «справка молчит», contradicted — «справка говорит иначе». В вымышленной истории и в условном примере (philosophy, explainer) постановочный механизм допустим: это постановка, а не утверждение о реальном событии.
...
У каждой задачи: id; learns — что зритель узнаёт, по-русски одной фразой (у объяснения — что именно объясняет голос); role; номера фраз; action — одно выполнимое наблюдаемое действие по-английски.
role:
...
- "explanation" — правовая, числовая или причинная связь, для которой доказательства в кадре нет. Смысл несёт голос автора. Под неё допустима иллюстрация, которая помогает пониманию и не выдаёт себя за доказательство: место, обстановка, масштаб, люди в своей среде. Нельзя документ, штамп, чиновника, экран с цифрами — это выдуманное доказательство. Если помогающей картинки нет, action пустой и отрезок остаётся автору.
Порядок:
...
3. Каждая следующая задача добавляет к уже показанному новое понимание. Если нового показать нечего, это explanation. Метка explanation — не способ снять с себя показ: событие с предметом (разрыв, вскрытие, передача, нажатие) обязано быть показано, объяснением может быть только право, статус, сумма или причина.
...
- Каждая AI-сцена ссылается на свою визуальную задачу через visualTask. Сцены без задачи не бывает. Несколько сцен одной задачи допустимы, только если каждая следующая добавляет новое: общий план и деталь вместе, а не тот же момент с другого ракурса.
...
У КАЖДОГО ИЗМЕНЕНИЯ ДОЛЖНА БЫТЬ ВИДИМАЯ ПРИЧИНА В ЭТОМ ЖЕ КАДРЕ. Рука, контакт, нажатие, удар, поток, инструмент — что-то в кадре обязано это сделать. Результат, который просто появляется (экран сам сменился, сено само оказалось скошенным, устройство само подтвердило операцию), событием не считается: назови в действии того, кто и чем его вызывает, а в objects меняй состояние именно того предмета, к которому прикасаются.
Смена статуса, регистрации, права или договорённости не снимается сама по себе. Либо покажи её видимое следствие в момент, когда оно происходит (подпись ставится, табличку меняют, ключ переходит в другую руку), либо оставь это голосу автора и сними обстановку как обстановку. Прогулка вдоль готового результата — это контекст, а не событие.
...
- scene.props (список) — реквизит, который обязан быть в кадре: тот самый конверт, ключи на столе, инструмент в руке. Называй ПРЕДМЕТ и его постоянные приметы (цвет, материал, размер), но не его состояние в этой сцене: «оранжевый нейлоновый купол», а не «уже раскрытый купол». Фазу покажет действие;
...
- НАЗНАЧЕНИЕ РОЛИ И СОХРАНЕНИЕ ВНЕШНОСТИ — РАЗНЫЕ РЕШЕНИЯ. В документальной истории реальный человек другого возраста, пола или в другой одежде (подросток в худи, пожилой свидетель, полицейский в форме) — отдельный участник в supportingCharacters со своей внешностью; ${character.name} в такой сцене либо отсутствует, либо стоит рядом в своём костюме. В вымышленной истории и условном примере ${character.name} может играть такую роль собой, не меняя внешности. Ошибка назначения исправляется переназначением, а не переодеванием ${character.name}.
...
- длинные авторские куски — это explanation из visualTasks, а не места, где не нашлось сцены; и у каждого такого куска ты спросил себя, нет ли иллюстрации, которая помогла бы пониманию без доказательства.
