# Соцсети: Threads + X — личный аккаунт крипто-трейдера (раздел `/threads` сайта Гудини)

## Две площадки, одна лента (21 сентября 2026)

Всё пишется **от первого лица владельца** (персона в Настройках → Голос стоит перед каждым промптом) и уходит
на обе площадки из одного черновика: основной текст для Threads, короткий вариант без ссылок для X.

| Что публикуется | Откуда | Как |
| --- | --- | --- |
| **Сделки** | Hyperliquid, публичный info API по адресу кошелька (приватный ключ не нужен) | исполнения → сделки (усреднение, частичные закрытия, перевороты) → плечо и ROE → карточка 1440×810 с графиком → пост. Только закрытые **плюсовые** сделки выше порога; причина входа берётся лишь из заметки владельца |
| **Новости** | RSS и профили, прежний конвейер SOURCE → FACTS → NEW POST | факты проверяются, числа в тексте сверяются с фактами |
| **Рынок** | топ монет CoinGecko + funding/OI Hyperliquid | громкий взлёт/падение → пост из проверенных чисел; причина движения не выдумывается |
| **Свои темы** | поле «О чём написать» | механика без выдуманных событий |

Площадки равноправны в коде (`src/platforms`: адаптеры `threads` и `x`), но правила у них разные:

- **Threads** — Graph API: публикация, ответы под своими и чужими постами, поиск, метрики.
- **X** — собственный браузер: контейнер `x-browser` держит настоящий Chromium на виртуальном экране с входом
  владельца (так же устроен `gudini-tiktok-browser`). Вход выполняется один раз прямо из дашборда — кадры страницы
  идут картинками, клики и ввод возвращаются обратно, пароль попадает только в X. Ключей разработчика нет, за
  публикации и за чтение чужих постов никто не платит, ограничение на ответы чужим авторам снято: режим общения
  задаётся владельцем (`auto` — отвечать под постом, `manual` — готовить текст ему, `quote` — цитатой, `off` —
  не трогать). Дневной лимит чтения остался, но теперь это не счёт, а вежливость: X банит аккаунты, которые
  читают как скрипт. Ссылки в тексте по-прежнему вырезаются по умолчанию — владелец держит их в описании профиля.
  Запасной путь через платный API v2 включается строкой `X_TRANSPORT=api` и четырьмя ключами.
  Публикация идемпотентна: клик не повторяется вслепую, а сначала проверяется, не вышел ли пост уже.
- Черновик уходит на каждую площадку отдельно: дошло не везде → статус `PARTIAL`, повтор досылает только
  недостающее; кросс-пост считается одним постом в лимитах и интервалах.

Дашборд: Главная (что нужно от владельца, площадки, Hyperliquid) · Посты (редактор с превью обеих площадок) ·
Сделки · Рынок · Ответы мне · Чужие посты · Настройки. Проверено: `npm run typecheck`, 78 unit-тестов,
7 интеграционных сценариев (реальные тестовые Postgres/Redis; Threads, X, Hyperliquid и LLM подставные).
...
Проверено: `npm run typecheck`, 57 unit-тестов, 4 интеграционных сценария с реальными тестовыми Postgres/Redis и подставным Threads API. В тестах не публикуются реальные комментарии. На работающем сайте проверены реальное написание поста Claude через локальный мост и отображение на ширине 390 px.
