...
РАСПРЕДЕЛЕНИЕ ПО РОЛИКУ — ЭТО ЧАСТЬ ЗАДАЧИ.
- ПЕРВЫЙ бит ролика — full_ai, если в первых фразах есть хоть что-то показуемое. Ролик, который начинается с двадцати секунд говорящей головы, зритель закрывает: показывать надо с первой секунды.
- Между двумя сценами не оставляй больше ${MAX_AUTHOR_STRETCH_SEC} секунд подряд одного автора. Длинный кусок объяснения разбивай сценой в середине.
- Сцены распределяй по всей длине речи, а не собирай в конце. Развязка не должна быть единственным, что показано.
- При этом не превышай общий ориентир покрытия сверху: лучше несколько коротких сцен в разных местах, чем две длинные подряд.
